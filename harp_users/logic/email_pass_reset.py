...
from harp_users.logic.email_pass_reset_template import template
from harp_users.logic.get_bot_config import bot_config

logger = get_logger()


class PasswordResetEmail(object):
    ...
    def define_invite_confirmation_url(self):
        if settings.DOCKER_SERVER_IP:
            confirmation_url = f"http://{settings.DOCKER_SERVER_IP}/#/user-password-reset?token={self.token}&username={self.username}"
        else:
            confirmation_url = f"https://{settings.SERVICE_NAMESPACE}.harpia.io/#/user-password-reset?token={self.token}&username={self.username}"
    ...
    def prepare_email(self):
        ...
        msgroot['Subject'] = 'Reset password confirmation'
...
# some_class = CreateEmail(recipient_name='mykola.kon2021@gmail.com', token='ololo', username='nkondratyk')
